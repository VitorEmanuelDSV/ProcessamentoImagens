...
def apply_clamping(pixel_matrix):
    ...
    for y in range(height):
        for x in range(width):
            val = pixel_matrix[y][x]
            if val < 0: output_matrix[y][x] = 0
            elif val > 255: output_matrix[y][x] = 255
            else: output_matrix[y][x] = val
    return output_matrix
...
def apply_high_boost_filter(pixel_matrix, factor_a):
    ...
    boosted_image = matrix_utils.add_matrices(pixel_matrix, scaled_high_pass)
